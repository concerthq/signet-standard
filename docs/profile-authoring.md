...
**Status:** Working Draft · **Tier:** 1 · **Applies to:** v0.16.0 and later
...
Point `conformance/rules/check-state-model.js` at your own registry and part of this list is
machine-checkable at v0.16.0:

- **C5** — no transition originates from a state declared terminal.
- **C9** — every declared state is reachable from the object's creation entry.
- **C4** — exactly one creation entry per lifecycle-bearing object.
- **C7** — every entry carries a `basis`.

The rest are by inspection for now. Two further checks — that every object can *reach* a terminal
state, and that no state value is a relation — are specified but not yet in force; they arrive with
a record still under comment at the time of writing. Both found defects in the standard's own
artifacts when run, which is the argument for writing your registry so they will pass.
