...
| **AP-5** | **No personal data in the chain** | Person references in hash-anchored records are pseudonymous identifiers, so the trail is both integrity-preserving and erasable. This requirement depends on the identity extension, currently a Working Draft; it is provisional and will be confirmed or revised when that extension is promoted. | Identity extension no-PII rule (§ "No personal data in hash-anchored records"). |
