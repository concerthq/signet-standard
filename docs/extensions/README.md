...
|----|--------------|--------|------|
| `onboarding` | `OnboardingCase`, `SupplierQualification` (durable status with first-class `conditional` qualification, value caps, category restrictions); `credentialType` codelist | **Schemas + demo shipped** (in-tree, core `v0.1`) | — (spec not yet in-tree) |
