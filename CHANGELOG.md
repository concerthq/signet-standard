...
- SE-0002 filed through the scope-enquiry route as [issue #62](https://github.com/concerthq/signet-standard/issues/62) — what `Contract` denotes in v1, and whether organisational units are in scope. Two registered questions, no proposal. Same route SE-0001 took ([#49](https://github.com/concerthq/signet-standard/issues/49)); the mapping from SE number to issue is recorded here because the records cross-reference `SE-0002` by name.
...
- `governance/IAR-0007-founding-adopter-window.md` — interim resolution establishing the register and a founding-adopter window that closes with IAR-0006 (constitution, or 23:59 UTC 30 September 2026). Founding status is a fact about timing and confers nothing. Eight declined alternatives recorded. Tier 1.
