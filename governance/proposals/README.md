...
| [CP-Requirement-Sockets](CP-Requirement-Sockets.md) | A score cannot name its criterion, an obligation cannot say where it came from, a submission cannot state a position on what was asked (D-34..D-36) | No | No | Draft; registered under IAR-0006 |
| [CP-Extension-Composition-Amendment-A](CP-Extension-Composition-Amendment-A.md) | Amends CP-Extension-Composition: published-extension fields compose under the Part 1 pattern via the harness; narrows Part 2 (gates E-1/E-3; D-37, D-38) | No change to Part 1; narrows Part 2 | No | Draft; registered under IAR-0006 |
...
| [CP-Qualification-Lifecycle](CP-Qualification-Lifecycle.md) | `SupplierQualification` has one `status` for a determination that is staged and category-scoped in practice, and the model hard-codes qualify-before-bid (D-57 second surface; EAE-0001 #2, #3a) | Undecided by design — the gates decide | No | Draft; registered under IAR-0006 |
| [CP-Auction-Determinism](CP-Auction-Determinism.md) | The auction close is defined over bids rather than the ordered event history; round, step, tie, seed and money rules are under-specified; the event hash exists only in code and chain scope is defined two ways (D-62..D-71; EAE-0002) | Core in form only (`previousEventHash` pattern, no known instance); extension yes (reserved combinations, seedless `random-seeded`) | No | Draft; registered under IAR-0006. Adoption by the Committee only (interests register entry 5) |
...
| CP-Requirement-Sockets | the next v0.x minor after registration | `schema/definitions.schema.json`, `schema/policy.schema.json`, `schema/obligation.schema.json`, `schema/submission.schema.json`, `codelists/`, `codelists/bindings.json`, `conformance/rules/`, `conformance/fixtures/`, `docs/specification.md`, `wiki/Process-Layer.md`, `wiki/Codelists.md` | none. Independent of CP-Extension-Composition Part 2 and of the `requirements` extension; each addition is justified on its own merits (§1) | `docs/extensions/requirements.md` moving from "spec only" to "schemas shipped" |
| CP-Extension-Composition-Amendment-A | with CP-Extension-Composition Part 2 (v1.0 train) | CP-Extension-Composition §1.1, §2 Part 2, §6 alternative B, §7 D5a, §8 E-1/E-3; CP-Codelist-Enforcement §7 C-4 | Part 1 (landed, v0.16.0, 22 sites) | Part 2 scope |
| CP-Commodity-Risk-Linkage | the next v0.x minor after registration | `docs/extensions/commodity-risk.md`, `examples/commodity-risk/`, `codelists/eventType.csv`, `codelists/policyType.csv`, `codelists/bindings.json`, `schema/coverage-assessment.schema.json`, `schema/coverage-policy.schema.json`, `schema/hedge-proposal.schema.json`, `state-model/state-model.json`, `conformance/rules/check-commodity-risk.js`, `wiki/Codelists.md` | none for §2.1, §2.3, §2.5. §2.2 takes a position on a question CP-EventType-Closure also answers (gate L-1); §2.4 relies on `Policy` staying as shipped | any conformance claim against the `commodity-risk` extension's rules 5 and 6; `docs/extensions/commodity-risk.md` moving from Working Draft |
| CP-Quantity-Unit | the next v0.x minor after registration | `schema/definitions.schema.json`, `schema/order.schema.json`, `schema/contract.schema.json`, `schema/exposure-position.schema.json`, `schema/hedge-proposal.schema.json`, `schema/price-mark.schema.json`, `schema/coverage-assessment.schema.json`, `examples/`, `conformance/rules/`, `docs/specification.md`, `docs/extensions/commodity-risk.md` §7, `wiki/Foundation-Layer.md`, `wiki/Process-Layer.md` | none. Independent of CP-Commodity-Risk-Linkage; the two touch different properties of the same extension schemas | any energy-supply `Order` or `Contract` in core; `PriceMark` graduation (a graduated object must not carry a free-text unit) |
| CP-Market-Identifiers | the next v0.x minor after registration | `codelists/identifierScheme.csv`, `schema/exposure-position.schema.json`, `schema/coverage-assessment.schema.json`, `schema/hedge-proposal.schema.json`, `schema/price-mark.schema.json`, `schema/coverage-policy.schema.json`, `examples/commodity-risk/`, `docs/extensions/commodity-risk.md`, `wiki/Codelists.md` | none. `CP-Quantity-Unit` §2.3 references the scheme this CP adds but does not require it | nothing in core; an energy implementer's ability to identify a market area, metering point or market participant by the scheme the sector uses |
| CP-Qualification-Lifecycle | undecided — the gates decide the shape before a release can be named | `schema/supplier-qualification.schema.json`, `state-model/state-model.json` (`SupplierQualification` is declared `modelled: false`), `docs/extensions/onboarding.md`, `onboarding/` | D-57's resolution (QL-2 must resolve consistently with it — same question, second surface); the D-39 extension-space pattern | nothing shipped; it blocks any state-model declaration for `SupplierQualification`, whose `modelled: false` rationale names the basis QL-3 supplies |
| CP-Auction-Determinism | the first v0.x minor after Committee adoption | `docs/extensions/auction.md`, `docs/profiles/auction-platform.md`, `docs/specification.md`, `conformance/adapter/adapter-contract.md`, `schema/auction.schema.json`, `schema/event.schema.json`, `codelists/eventType.csv`, new `codelists/bidRejectionReason.csv` and `codelists/ineligibilityReason.csv`, `state-model/state-model.json`, `conformance/rules/`, `conformance/fixtures/auction/`, `conformance/vectors/event-hash/`, `conformance/runner/lib.js`, `auction/`, `examples/auction-reverse.json`, `public-interface.json`, `wiki/` | none; shaped around D-20 and D-39 without resolving either | certification against `auction-platform` beyond the pinned release; `docs/extensions/auction.md` leaving Working Draft |
